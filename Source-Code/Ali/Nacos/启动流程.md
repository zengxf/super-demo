...
public class ExternalConfigInfoPersistServiceImpl implements ConfigInfoPersistService {
    ...
    public ExternalConfigInfoPersistServiceImpl(
        ...
        this.mapperManager = MapperManager.instance(isDataSourceLogEnable); // 获取 Mapper 管理器单例
...
public class ExternalDataSourceProperties {
    ...
    List<HikariDataSource> build(Environment environment, Callback<HikariDataSource> callback) {
        ...
        // 使用 Spring 工具类，将环境变量绑定到自身属性 (去掉 'db' 前缀)
        ...
        for (int index = 0; index < num; index++) {
            ...
            ... // 省略 url 校验
            DataSourcePoolProperties poolProperties = DataSourcePoolProperties.build(environment);  // sign_m_440
